...
class fred_api:
    ...
    def observations(self):
        ...
        if type(self.series_id) == list:
            ...
            self.file_type = 'json'
            self.url = (root_url + self.series_id + '&api_key='
                        + self.FRED_API_Key
                        + '&file_type=' + self.file_type)
...
def yahoo_sp(stock_symbol):
    ...
    if yahoo_url_text.find('Download to Spreadsheet') != -1:
        ...
        stock_df['close_date'] = p.to_datetime(stock_df['close_date'])
        
        ##Add a symbol column
        stock_df['symbol'] = stock_symbol

        ##Create Index with Symbol and Date
        stock_df.set_index(['symbol','close_date'],
                           inplace=True,
                           drop=True)
...
def yahoo_div(stock_symbol):
    ...
    if yahoo_url_text.find('Download to Spreadsheet') != -1:
        ...
        ##Add a date column
        stock_df['div_date'] = stock_df.index
        stock_df['div_date'] = p.to_datetime(stock_df['div_date'])

        ##Add a symbol column
        stock_df['symbol'] = stock_symbol

        ##Create Index with Symbol and Date
        stock_df.set_index(['symbol','div_date'],
                           inplace=True,
                           drop=True)
...
def short_int(stock_symbol):
    ##Stock price information from Nasdaq.
    url = ('http://www.nasdaq.com/symbol/' + stock_symbol + '/short-interest')
    
    try:
        stock_df = p.read_html(url, match='Days To Cover')
    except:
        stock_df = p.DataFrame()
        return stock_df
    ...
    new_col_names = ['settlement_date',
                     'short_interest',
                     'avg_daily_share_volume',
                     'days_to_cover']

    stock_df.columns = new_col_names

    ##Add a date and symbol index column
    stock_df['settlement_date'] = p.to_datetime(stock_df['settlement_date'])
    stock_df['symbol'] = stock_symbol
    
    stock_df.set_index(['symbol','settlement_date'],
                       inplace=True,
                       drop=True)
