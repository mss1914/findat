...
class lin_reg:
    ...
    def sim(self, nsims):
        ...
        for n in range(0,self.nsims):
            ...
            r_chi_var = float(ss.chi2.rvs(self.df, size=1))
            ...
            sim_beta = ss.multivariate_normal.rvs(sim_beta_mean, sim_beta_var)
        ...
        self.sim_result = [self.b_hat_sims, self.sigma_hat_sims]
        return self.sim_result

    def pred_sim(self,nsims,pred_variables):
        ##Check to make sure that Pandas Series is used for pred_variables
        if type(pred_variables) != p.core.series.Series:
            raise TypeError("'pred_variables' argument must be a Pandas Series")
        ##Check to make sure that int is used for nsims
        elif type(nsims) != int:
            raise TypeError("'nsims' argument must be an integer")
        else:
            ##Designed to use the "sim" function to predict confidence intervals
            pv = p.Series.tolist(pred_variables)
            ...
            for pred_value in pv:
                ##Get the estimate of the pred_value
                ...
                for loop in range(nsims):
                    np.put(ran_vec,
                           loop,
                           ss.norm.rvs(loc=0,scale=sigma[loop]))
                ##Append random variables vector to list.
                array_list.append(ran_vec + point_est)
            ##Create one matrix from all vectors.
            pred_sim_result = np.vstack(array_list)
            return pred_sim_result
...
def log_norm_mc_chain(start,nsims,periods,loc,scale):
    ...
    for sim in range(nsims):
        ...
        for period in range(periods):
            rv = ss.norm.rvs(loc=loc,scale=scale)
            np.put(temp_vec,
                   period,
                   temp_start+rv)
